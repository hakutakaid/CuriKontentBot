...
async def _batch(event):
    ...
    if session_data:
      ...
      await event.respond("Login di bot untuk melanjutkan pengiriman /login atau untuk pengiriman /pengaturan berbasis sesi")
      return

    if user_id in batch_data:
        return await event.reply("Anda sudah memulai satu batch, tunggu sampai selesai!")

    async with gagan.conversation(event.chat_id) as conv: 
        try:
            await conv.send_message(f"Kirimi saya tautan pesan tempat Anda ingin mulai menyimpan, sebagai balasan atas pesan ini.", buttons=Button.force_reply())
            ...
            try:
                ...
                return
            await conv.send_message(f"Kirimi saya jumlah file/rentang yang ingin Anda simpan dari pesan yang diberikan, sebagai balasan atas pesan ini.", buttons=Button.force_reply())
            _range = await conv.get_reply()
            try:
                value = int(_range.text)
                if value > 1000:
                    return await conv.send_message("Anda hanya bisa mendapatkan hingga 1000 file dalam satu batch.\n\nBeli premium melebihi batas pengiriman /rencanakan untuk mengetahui lebih banyak...")
            except ValueError:
                return await conv.send_message("Rentang harus berupa bilangan bulat!")
            ...
            cd = await conv.send_message("**Proses batch sedang berlangsung...**\n\nProses selesai:", 
                                    buttons=[[Button.url("Join Channel", url="http://t.me/AnonymousX888")]])
            co = await run_batch(userbot, Bot, user_id, cd, _link) 
            try: 
                if co == -2:
                    await Bot.send_message(user_id, "Batch berhasil diselesaikan!")
                    await cd.edit(f"**Proses batch sedang berlangsung.**\n\nProses selesai: {value} \n\n Batch berhasil diselesaikan! ")
            except:
                await Bot.send_message(user_id, "ERROR!\n\n mungkin pesan terakhir belum ada")
            finally:
                conv.cancel()
...
async def cancel_command(event):
    ...
    if str(user_id) in ids_data:
        del ids_data[str(user_id)]
        save_ids_data(ids_data)
        await event.respond("Operasi dibatalkan.")
    else:
        await event.respond("Tidak ada operasi untuk dibatalkan.")
...
async def run_batch(userbot, client, sender, countdown, link):
    for i in range(len(ids_data[str(sender)])):
        ...
        try: 
            count_down = f"**Proses batch sedang berlangsung.**\n\nProses selesai: {i+1}"
            integer = int(link.split("/")[-1]) + int(ids_data[str(sender)][i])
            await get_bulk_msg(userbot, client, sender, link, integer)
            protection = await client.send_message(sender, f"Tidur selama `{timer}` detik untuk menghindari Floodwait dan Lindungi akun!")
            await countdown.edit(count_down, 
                                 buttons=[[Button.url("Join Channel", url="https://t.me/AnonymousX888")]])
            ...
            if int(fw.value) > 300:
                await client.send_message(sender, f'Anda memiliki waktu tunggu selama {fw.value} detik, membatalkan batch')
                ids_data.pop(str(sender))
                break
            else:
                fw_alert = await client.send_message(sender, f'Tidur selama {fw.value + 15} detik karena menunggu banjir di Telegram.')
                ors = fw.value + 5
                ...
                try:
                    ...
                    if countdown.text != count_down:
                        await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="http://t.me/AnonymousX888")]])
        except Exception as e:
            #logger.info(e)
            #await client.send_message(sender, f"An error occurred during cloning, batch will continue.\n\n**Error:** {str(e)}")
            if countdown.text != count_down:
                await countdown.edit(count_down, buttons=[[Button.url("Join Channel", url="https://t.me/AnonymousX888")]])
        n = i + 1
