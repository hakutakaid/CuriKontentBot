...
from curkontent.assets.progress import progress_for_pyrogram
from curkontent.assets.functions import screenshot
from pyrogram import Client
from pyrogram.types import Message
from pyrogram.errors import ChannelBanned, ChannelInvalid, ChannelPrivate, ChatIdInvalid, ChatInvalid
from curkontent.assets.functions import video_metadata
...
async def copy_message_with_chat_id(client, sender, chat_id, message_id):
    ...
    try:
        ...
        caption = f"{final_caption}\n\n__**{custom_caption}**__" if custom_caption else f"{final_caption}\n\n__**[Team SPY](https://t.me/AnonymousX888)**__"
...
async def send_video_with_chat_id(client, sender, path, caption, duration, hi, wi, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_video(
            ...
            progress_args=(
                client,
                '**__Uploading: [Team SPY](https://t.me/AnonymousX888)__**\n ',
                upm,
                time.time()
            )
        )
...
async def send_document_with_chat_id(client, sender, path, caption, thumb_path, upm, is_pinned):
    ...
    try:
        result = await client.send_document(
            ...
            progress_args=(
                client,
                '**__Uploading:__**\n**__Bot made by [Team SPY](https://t.me/AnonymousX888)__**',
                upm,
                time.time()
            )
        )
...
async def get_msg(userbot, client, sender, edit_id, msg_link, i, file_n):
    ...
    if 't.me/c/'  in msg_link or 't.me/b/' in msg_link:
        ...
        try:
            ...
            file = await userbot.download_media(msg, progress=progress_for_pyrogram, progress_args=(client, "**__Unrestricting__: __[Team SPY](https://t.me/AnonymousX888)__**\n ", edit, time.time()))            # Retrieve user's custom renaming preference if set, default to '@AnonymousX888' otherwise
            if not file:
...
def load_replacement_words(user_id):
    try:
        ...
        print(f"Terjadi kesalahan saat memuat kata pengganti: {e}")
        return {}
...
def save_replacement_words(user_id, replacements):
    try:
        ...
        print(f"Terjadi kesalahan saat menyimpan kata pengganti: {e}")
...
def get_user_rename_preference(user_id):
    # Retrieve the user's custom renaming tag if set, or default to '@AnonymousX888'
    return user_rename_preferences.get(str(user_id), '@AnonymousX888')
...
MESS = "Sesuaikan menurut keinginan Anda dan Konfigurasikan pengaturan Anda..."
...
async def settings_command(event):
    buttons = [
        ...
        [Button.url("Report Errors", "https://t.me/AnonymousX888")]
    ]
...
async def callback_query_handler(event):
    ...
    if event.data == b'setchat':
        await event.respond("Kirimi saya ID obrolan itu:")
        sessions[user_id] = 'setchat'

    elif event.data == b'setrename':
        await event.respond("Kirimi saya tag ganti nama:")
        sessions[user_id] = 'setrename'

    elif event.data == b'setcaption':
        await event.respond("Kirimi saya keterangannya:")
        sessions[user_id] = 'setcaption'

    elif event.data == b'setreplacement':
        await event.respond("Kirimkan saya kata-kata pengganti dengan format: 'WORD(s)' 'REPLACEWORD'")
        sessions[user_id] = 'setreplacement'

    elif event.data == b'addsession':
        await event.respond("Metode ini disusutkan ... gunakan /login")
        ...
        await event.respond("Kirim kata-kata yang dipisahkan spasi untuk menghapusnya dari keterangan/nama file ...")
        sessions[user_id] = 'deleteword'
        
    elif event.data == b'logout':
        result = mcollection.delete_one({"user_id": user_id})
        if result.deleted_count > 0:
          await event.respond("Keluar dan sesi berhasil dihapus.")
        else:
          await event.respond("Anda belum masuk")   

    elif event.data == b'setthumb':
        pending_photos[user_id] = True
        await event.respond('Silakan kirimkan foto yang ingin Anda jadikan thumbnail.')

    elif event.data == b'remthumb':
        try:
            os.remove(f'{user_id}.jpg')
            await event.respond('Gambar mini berhasil dihapus!')
        except FileNotFoundError:
            await event.respond("Tidak ditemukan thumbnail yang perlu dihapus.")
...
async def save_thumbnail(event):
    ...
    if event.photo:
        ...
        await event.respond('Thumbnail berhasil disimpan!')

    else:
        await event.respond('Silakan kirim foto... Coba lagi')
    ...
    if user_id in sessions:
        ...
        if session_type == 'setchat':
            try:
                ...
                await event.respond("ID Obrolan berhasil ditetapkan!")
            except ValueError:
                await event.respond("ID obrolan tidak valid!")
